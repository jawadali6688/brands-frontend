...
const Unstiched = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch user's favorite unstiched products
        if (userId) {
  ...
  // Filter products based on active filters (color, size, price)
  useEffect(() => {
    let filtered = products;

    if (activeColor) filtered = filtered.filter((product) => product.color === activeColor);
    if (activeSize) filtered = filtered.filter((product) => product.size === activeSize);
  ...
  const toggleFavorite = async (productId) => {
    try {
      ...
      if (isFavorite) {
        ...
        const updatedFavorites = response?.data?.favourites?.favourites;
  ...
  return (
    <div className="max-w-7xl mx-auto p-8">
      <h2 className="text-3xl font-bold mb-6">FootWear</h2>
