...
const Shirts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch shirts
        const response = await axios.get("http://localhost:8000/api/products/shirts");
            ...
            <div className="flex justify-between mt-2">
              <FaHeart
                className={`text-4xl hover:cursor-pointer ${
                  favorites?.some((item) => item?._id === product?._id) ? "text-red-600" : "text-gray-500"
                }`}
