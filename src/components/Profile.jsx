...
const Profile = () => {
  ...
  useGetUserProfile(userId); // Fetch the user profile based on userId
  const [activeTab, setActiveTab] = useState('posts'); // To handle the active tab

  // Extracting the authenticated user and the userProfile being viewed from the Redux store
  const { userProfile, user } = useSelector((store) => store.auth);

  // Check if the logged-in user is viewing their own profile
  const isLoggedInUserProfile = user?._id === userProfile?._id;
  const isFollowing = false; // Set following state dynamically when implemented

  // Switch between "posts" and "saved" content
  ...
  return (
    <div className="flex max-w-5xl justify-center mx-auto pl-10">
      <div className="flex flex-col gap-20 p-8">
        ...
        {/* Tabs to switch between posts, saved, reels, and tags */}
        <div className="border-t border-t-gray-200">
          ...
          {/* Grid to display the posts or saved items */}
          <div className="grid grid-cols-3 gap-1">
            {displayedPost?.map((post) => (
              <div key={post?._id} className="relative group cursor-pointer">
                {post.mediaUrl && post.mediaUrl.endsWith('.mp4') ? (
                  <video controls className="rounded-sm my-2 w-full aspect-square object-cover">
